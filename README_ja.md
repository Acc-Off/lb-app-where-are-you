...
本リソースは以下に依存します。`server.cfg` 上で `lb-app-where-are-you` より**先に起動**するよう、インストール・配置してください。

| リソース   | 最低バージョン | 補足                                                                 |
| ---------- | -------------- | -------------------------------------------------------------------- |
| `lb-phone` | **v2.7.0**     | 地図に lb-phone 内蔵の `GameMap` コンポーネント（v2.7.0 で追加）を使用します。これより古いと地図が表示されません。 |
| `ox_lib`   | 最新           | UI の多言語化（`lib.getLocales()`）に使用します。未導入または古いと、UI のテキストがすべてキー名（例: `timeline.post_btn`）のまま表示されます。 |
| `oxmysql`  | 最新           | DB アクセスに使用します。この DB に対して `sql.sql` を実行してください（下記手順2）。 |
...
> **注意:** 下の方にある `### 依存関係インストール` の手順は React 製 UI のビルド用（`npm install`）であり、ここで挙げたサーバー側の依存リソースとは別物です。
